document.addEventListener('DOMContentLoaded', () => {
  ...
  function createBoard() {
    ...
    // Remplir le tableau avec des valeurs aleatoires de vrai ou faux
    for(let i = 0; i < nb_lignes; i++) {
      grille[i] = []
      for(let j = 0; j < nb_colonnes; j++) {
        grille[i][j] = Math.random() >= 0.5 ? true : false
      }
    }

    for (const unTableau of grille) {
      for(const uneCase of unTableau ) {
